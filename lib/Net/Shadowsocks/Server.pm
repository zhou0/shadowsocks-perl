                                                  ...
                                                  on_read => sub 
                                                  {
                                                      ...
                                                      elsif($self->{method} =~ /^chacha20/)
                                                      {
                                                          ...
                                                          else
                                                          {
                                                              ...
                                                              my $data_len_ct_withtag = $encryptor ->ietf_encrypt($data_len_pt,undef,$encrypt_nonce,$encrypt_subkey);
                                                              $encrypt_nonce = $encrypt_nonce->increment();
                                                              my $data_ct_withtag  =  $encryptor->ietf_encrypt($plain_data,undef,$encrypt_nonce,$encrypt_subkey);
                                                              $encrypt_nonce = $encrypt_nonce->increment();
                                       ...
                                       else
                                       {
                                           while (length($incoming_data) > 0)
                                           {
                                               ...
                                               else
                                               {
                                                            ...
                                                            eval
                                                            {
                                                                $data_len_pt = $decryptor->ietf_decrypt($data_len_ct, undef, $decrypt_nonce, $decrypt_subkey);
                                                            };
                                                            if ( $@ ) 
                                                            {
                                                                AE::log error =>  "data length forged!";
                                                            ...
                                                            else 
                                                            {
                                                                ...
                                                                {
                                                                    $decrypt_nonce = $decrypt_nonce->increment();
                                                                    $decrypted_data .= $decryptor->ietf_decrypt(substr($incoming_data,18,$data_len + 16),undef,$decrypt_nonce,$decrypt_subkey);
                                                                     if ( $@ ) 
                                                                     {
                                                                         AE::log error =>  "data forged!";
                                                                         $chandle->destroy();
