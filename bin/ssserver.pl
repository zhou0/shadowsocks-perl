...
# server, run ssclient.pl on localhost. Configure your browser to use socks 5
# proxy with remote DNS support and you can now break through the great firewall
# of china.
...
{
    print("Usage: ssserver.pl -s SERVER_ADDR -p SERVER_PORT [-b LOCAL_ADDR]
    ...
    print("\taes-128-cbc aes-128-cfb aes-128-ctr aes-128-ofb\n");
    print("\taes-192-cbc aes-192-cfb aes-192-ctr aes-192-ofb\n");
    print("\taes-256-cbc aes-256-cfb aes-256-ctr aes-256-ofb\n");
    print("\tcamellia-128-cbc camellia-128-cfb camellia-128-ctr camellia-128-ofb\n");
    print("\tcamellia-192-cbc camellia-192-cfb camellia-192-ctr camellia-192-ofb\n");
    print("\tcamellia-256-cbc camellia-256-cfb camellia-256-ctr camellia-256-ofb\n");
    print("\tchacha20-ietf rc4-md5 rc4-sha rc6\n");
...
if (!defined($options{k}) or !defined($options{p}) or !defined($options{s}) )
{
    if(!defined($options{c}))
    {
        print"You did not provide server, server port or password command argument, try again.\n";
        main::HELP_MESSAGE();
    }
    else
    {
...
Shadowsocks protocol server.

Usage: ssserver.pl -s SERVER_ADDR -p SERVER_PORT [-b LOCAL_ADDR]
    -l LOCAL_PORT -k PASSWORD -m METHOD [-c CONFIG]
	...
	-c CONFIG	Name of your config.json file with full path
